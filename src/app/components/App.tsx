    ...
    return new Promise(function(resolve, reject) {
        ...
        xhr.onload = function() {
            if (xhr.status > LAST_SUCCESSFUL_STATUS) {
                setLoading(false);
                setErrorLog(
                    `Ошибка в запросе ${requestType} ${url}: ${xhr.status} ${JSON.parse(xhr.responseText).message}`,
                );
                reject();
            } else {
            ...
            if (event.data.pluginMessage.type === 'networkRequest') {
                ...
                httpRequest(
                    ...
                    .then(() => {
                        ...
                        return httpRequest(config, 'POST', `${config.repoPath}/pulls`, setErrorLog, setLoading, [
                            JSON.stringify({
